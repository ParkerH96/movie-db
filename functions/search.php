      ...
      if(!empty($_GET['search'])){
        if(!empty($_GET['genre'])) {
          ...
          foreach ($genre as $genre_values) {
            $genre_query .= " AND genre LIKE '%$genre_values%'";
          }
        ...
        if($_GET['option'] === 'Any'){
          ...
          */
          ...
          if($_GET['option'] === 'Title'){
            ...
            $search_query = $mysqli->query("SELECT * FROM MOVIE, GENRE, is_genres WHERE title LIKE '%$search_key%' . '%$genre_query%'");

          }
          else if($_GET['option'] === 'Genre'){

            $search_query = $mysqli->query("SELECT * FROM MOVIE, GENRE, is_genres WHERE MOVIE.movie_id = is_genres.movie_id AND is_genres.genre_id = GENRE.genre_id AND genre LIKE '%$search_key%' . '%$genre_query%' GROUP BY (MOVIE.movie_id)");

          }
          else if($_GET['option'] === 'Tag'){

            $search_query = $mysqli->query("SELECT * FROM MOVIE, TAGS, GENRE, is_genres has_tags WHERE MOVIE.movie_id = has_tags.movie_id AND has_tags.tag_id = TAGS.tag_id AND tag LIKE '%$search_key%' . '%$genre_query%' GROUP BY (MOVIE.movie_id)");
        ...
        $genres = $_GET['genre[]'];
        if(isset($genres)) {
              $genre_query .= "AND genre LIKE 'Comedy'";
        }

      $search_query = $mysqli->query("SELECT * FROM MOVIE, GENRE, is_genres WHERE MOVIE.movie_id = is_genres.movie_id AND is_genres.genre_id = GENRE.genre_id  $genre_query GROUP BY (MOVIE.movie_id)");
      if($search_query){
